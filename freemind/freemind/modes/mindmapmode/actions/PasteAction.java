...
public class PasteAction extends AbstractAction implements ActorXml {
    ...
    private MindMapNode createNodeFromLine(String lineContent, MindMap map, java.util.List<String> outShortcuts) {
 		...
 		if (iconListMatcher.find()) {
 			// Ícones encontrados: [icone1, icone2, ...]
 			String allShortcuts = iconListMatcher.group(1);
 			String[] shortcutArray = allShortcuts.split(",");
 			String restOfLine = lineContent.substring(iconListMatcher.end());

 			boolean preserveSpaces = false;

 			// Verifica se 'v' ou 'y' estão presentes para preservar espaços e preenche a lista de atalhos.
 			for (String s : shortcutArray) {
 				String trimmedShortcut = s.trim();
 				if (!trimmedShortcut.isEmpty()) {
 					outShortcuts.add(trimmedShortcut);
 					if ("v".equals(trimmedShortcut) || "y".equals(trimmedShortcut)) {
 						preserveSpaces = true;
 					}
 				}
 			}

 			// Define o texto do nó com base na regra de preservação de espaços.
 			if (preserveSpaces) {
 				nodeText = restOfLine;
 			} else {
 				nodeText = restOfLine.trim();
 			}

 			// Cria o nó.
 			node = mMindMapController.newNode(nodeText, map);

 			// Adiciona todos os ícones encontrados ao nó.
 			for (String s : outShortcuts) {
 				String iconName = mapperIdeaIconMap.get(s);
 				if (iconName == null) {
 					iconName = s; // Fallback para usar o nome diretamente.
 				}
 			...
 			// Nenhum ícone encontrado, trata como uma linha de texto simples.
    ...
    private MindMapNode pasteStringWithoutRedisplay(Transferable t,
        ...
        for (int i = 0; i < textLines.length; ++i) {
            ...
            // Nova lógica: chama o método auxiliar para criar o nó e popular os ícones.
            java.util.List<String> foundShortcuts = new java.util.ArrayList<String>();
            MindMapNode node = createNodeFromLine(lineContent, parent.getMap(), foundShortcuts);
            String shortcut = null;
            if (!foundShortcuts.isEmpty()) {
                shortcut = foundShortcuts.get(0);
            }
